...
This repository is the **open-source LangChain integration** for the Unison brain.
...
- credential handling of `UNISON_TOKEN` in the local environment,
- how the token is passed to the Unison API (HTTP headers, not query strings),
...
variable or from the `token=` constructor argument. The token is never logged,
written to disk, or transmitted anywhere except the configured `UNISON_API_URL`
host. It is sent as an `Authorization: Bearer` HTTP header on every request.

Never commit `UNISON_TOKEN` to version control. Use `.env` files (listed in
`.gitignore`) or your CI provider's secrets store.
